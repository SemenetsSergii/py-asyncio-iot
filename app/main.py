...
async def main() -> None:
    ...
    # run the programs
    start_time = asyncio.ensure_future(service.run_program(wake_up_program))
    end_time = asyncio.ensure_future(service.run_program(sleep_program))
    await asyncio.gather(start_time, end_time)
